...
all.data <- all.data %>% 
  mutate(percent.voted = total.voted / registered.voters * 100)
...
ggplot(data = all.data) +
  geom_point(
    mapping = aes(x = total.population, y = percent.voted),
    color = 'blue'
  )
...
ggplot(data = all.data) +
  ...
  geom_text(aes(x = total.population, y = percent.voted, label=county, hjust = 1))
